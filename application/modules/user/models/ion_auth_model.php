...
class Ion_auth_model extends CI_Model
{
    ...
    private function initialize()
    {
	    ...
	    if(!$this->db->table_exists($tables["groups"]))
	    {
			...
			$this->dbforge->create_table($tables['groups']);
			...
			$this->db->insert_batch($tables['groups'], $data);
		}

	    if(!$this->db->table_exists($tables["users"]))
	    {
	    ...
	    if(!$this->db->table_exists($tables["login_attempts"]))
	    {
			...
			$this->dbforge->add_key('id', TRUE);
			$this->dbforge->create_table($tables['login_attempts']);
		}
	}
